...
namespace Hazel
{
	...
	namespace Utils
	{
		...
		static void AttachColorTexture(uint32_t id, int samples, GLenum internalFormat, GLenum format, uint32_t width, uint32_t height, int index)
		{
			...
			glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + index, TextureTarget(multsampled), id, 0);
		}
		...
		static GLenum HazelFBTexureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	int OpengGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
	{
	...
	void OpengGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		HZ_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(),"");
		auto& spec = m_ColorAttachmentSpecifications[attachmentIndex];
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::HazelFBTexureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
