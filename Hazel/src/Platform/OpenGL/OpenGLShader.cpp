...
namespace Hazel
{
	namespace Utils
	{
		...
		static const char* GLShaderStageCachedVulkanFileExtension(uint32_t state)
		{
			switch (state)
			{
		...
		:m_FilePath(filePath)
	{
		...
		auto shaderSources = PrePrecess(source);
		{
			...
			CreateProgram();
			HZ_CORE_WARN("Shader createion took {0} ms ", timer.ElapsedMillis());
		}
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			{
				shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, Utils::GLShaderStageToShader(stage), m_FilePath.c_str(), options);
	...
	void OpenGLShader::CompileOrGetOpenGLBinaries()
	{
		...
		for (auto&& [stage, spirv] : m_VulkanSPIRV)
		{
			...
			{
				spirv_cross::CompilerGLSL glslComiler(spirv);
				m_OpenGLSourceCode[stage] = glslComiler.compile();
				auto& source = m_OpenGLSourceCode[stage];

				shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(source, Utils::GLShaderStageToShader(stage), m_FilePath.c_str());
				if (module.GetCompilationStatus() != shaderc_compilation_status_success)
				{
					HZ_CORE_ERROR(module.GetErrorMessage());
					HZ_CORE_ASSERT(false);
